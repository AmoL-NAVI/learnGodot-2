...
func _process(delta: float) -> void:
	...
	var nex_path_position: Vector2 = nav_agent.get_next_path_position()
	...
	if event is InputEventMouseButton and event.is_pressed() and event.button_index == MOUSE_BUTTON_LEFT:
		if dialogue:
			...
			if type != NPCType.IDLE:
				await EventBus.on_dialogue_finsihed
...
func set_new_target() -> void:
	...
	var world_position: Vector2 = Refs.navigation.to_global(Refs.navigation.map_to_local(random_cell))
